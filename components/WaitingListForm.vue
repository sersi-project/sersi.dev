<template>
  <div class="flex flex-col items-center gap-2" v-if="submitted">
  ...
  <UForm :schema="schema" :state="state" @submit="onSubmit" class="flex flex-col gap-8 text-xl" v-else>
      ...
      <UFormField label="What best describes you." required>
          <UInputMenu v-model="state.role" :items="roles" size="xl" />
      </UFormField>
      <UButton
        block
        type="submit"
